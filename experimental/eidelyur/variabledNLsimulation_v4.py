...
#
# Version v3 started at 07/12/2019 was finished at 07/22/2019
# 
# In this vesion different plots are improved.
#
# Version v4 started at 07/22/2019
#
# This version besides some improvements includes the possibility to write results 
# of data in *.dat files (flag 'saveAllResultsFlag' = 1).
#
# All files are wittens to directory ~eidelyur/results_v4.
# Format of names of the files is as follows. 
# ----------------
# Files with particle distributions
# (symbol 'p' in the name of file means dot '.'):
# outputFileName =  'results_v4/xPx_yPy-turns{}'.format(int(turn))
# outputFileName += '_prtcls-{}'.format(numbPartcls)
# outputFileName += '_t-0p{:03d}'.format(int(1000.*t_crrnt))
# outputFileName += '_{}'.format(today)+'.dat'
#
# Format of data in these files:
# Standard title
# Line "      After turn     Number: t = Value"
# Each next line contents the values of
# Prtcl       x (cm)       x' (rad)        y (cm)       y' (rad)
# -----------------
# Files with data of results_1:
# outputFileName = 'results_v4/result-1_{}'.format(today)+'.dat'
#
# Format of data in this file:
# Standard title
# Each next line contents values of
#  Step   Turn        t          knll (m)       VQP (m^2)
# -----------------
# Files with data of results_2:
# outputFileName = 'results_v4/result-2_{}'.format(today)+'.dat'
#
# Format of data in this file:
# Standard title
# Further next lines content the values of
# Turn   LostedParticles
# After that each line contents the values of
# Point   Turn  R>R_init    R>2 R_init   R>3 R_init   R>4 R_init   R>5 R_init
#
...
def plotcoordDistr(bunchParticles,saveFigureFlag,turn,t_crrnt,saveDataFlag,totalTitle):
#    
...
# Another way - use gridspec
    ...
    x0Title = "After turn {:5d}".format(int(turn))+": t={:.3f}".format(t_crrnt)+ \
              "\n$R_{crrnt}=\sqrt{<R^2>}$ = "+"{:.3f} +- {:.3f} mm".format(x2y2avr,x2y2dAvr)
...
#        plt.savefig(fileName)
    ...
    if saveDataFlag == 1:
        ...
        try:
            ...
            print 'Problem to open output file "%s"' % outputFileName
    
        outputFile.write ('%s' % totalTitle)
        strTitle = "\n#\n#       After turn {: 5d}: t = {:.5f}\n#".format(int(turn),t_crrnt)
        strTitle +=  "\n################################################################"

        outputFile.write ('%s' % strTitle)
        strTitle = \
        "\n#\n# Prtcl       x (cm)       x' (rad)        y (cm)       y' (rad)\n"
        outputFile.write ('%s' % strTitle)
        for prtcl in range(numbPartcls):
            strLine = '  {: 05d}'.format(prtcl)+'  '+ \
                      '{: 12.6e}'.format(newCoordinates[0,prtcl])+'  '+ \
                      '{: 12.6e}'.format(newCoordinates[1,prtcl])+'  '+ \
                      '{: 12.6e}'.format(newCoordinates[2,prtcl])+'  '+ \
                      '{: 12.6e}'.format(newCoordinates[3,prtcl])
            outputFile.write ('%s\n' % strLine)

        outputFile.close()
        print 'Close the written output file "%s"' % outputFileName
    return

def plotTracks(tracksCoords,numberTracks,dataPointsCounter,saveDataAfterTurns):
#    
# Plot'numberTracks' tracks from 'tracksCoords'
#
# tracksCoords is 3D array - (totalTurns,particles,(x,y)):
#   first index - points of track;
#   second index - current number of track (= current number of particle; <=5 !);
#   third index - x- and y-coordinate of track
#
    trackColor = ['r','b','k','m','g']
    numbPoints = dataPointsCounter
...
#              format(meanX,stdX,meanY,stdY)
    ...
    if saveDataAfterTurns == 1:
        ax0.set_xlabel('Turn',color='m',fontsize=14)
    else:
        ax0.set_xlabel('Turn/{}'.format(saveDataAfterTurns),color='m',fontsize='14')
...
#              format(meanX,stdX,meanY,stdY)
    ...
    if saveDataAfterTurns == 1:
        ...
        ax1.set_xlabel('Turn/{}'.format(saveDataAfterTurns),color='m',fontsize='14')
...
# 'outputFlag'        - flag to output the strength of one of nonlinear lens after it's magnification 
#                      for current step,
...
def plotResult_1(knll_strength,t_strength,vertQuadMomentum,steps,totalTurns, \
                 saveToFileFlag,totalTitle):
#
# Plot
#
    ...
    if saveToFileFlag == 1:
        turnsPerStep = int(totalTurns/steps)
        today = date.today()
        outputFileName = 'results_v4/result-1_{}'.format(today)+'.dat'
        print 'outputFileName = ',outputFileName
        ...
        for step in range(steps):
            strLine = '  {: 5d}'.format(step)+' {:5d}'.format(int(step*turnsPerStep))+'  '+ \
                      '{: 12.6e}'.format(knll_strength[step])+'  '+ \
                      '{: 12.6e}'.format(t_strength[step])+'  '+ \
                      '{: 12.6e}'.format(vertQuadMomentum[step])
            outputFile.write ('%s\n' % strLine)
...
def plotResult_2(lostedParticles,prtclsOutOfArea,totalTurns,dataPointsCounter, \
                 totalPrtcl,roundAperture,initialRadius,saveToFileFlag,totalTitle):
    ...
    point = range(dataPointsCounter)
    plt.plot(point,prtclsOutOfArea[0:dataPointsCounter,0],'-x',color='r')
    plt.plot(point,prtclsOutOfArea[0:dataPointsCounter,1],'-x',color='b')
    plt.plot(point,prtclsOutOfArea[0:dataPointsCounter,2],'-x',color='g')
    plt.plot(point,prtclsOutOfArea[0:dataPointsCounter,3],'-x',color='m')
    plt.plot(point,prtclsOutOfArea[0:dataPointsCounter,4],'-x',color='k')
    if totalTurns == dataPointsCounter:
        ax_21.set_xlabel('Turn',color='m',fontsize=14)
    else:
        ax_21.set_xlabel('Turn/{}'.format(int(totalTurns/dataPointsCounter)),color='m',fontsize=14)
    ...
    if saveToFileFlag == 1:
        turnsPerPoint = int(totalturns/dataPointsCounter)
        today = date.today()
        outputFileName = 'results_v4/result-2_{}'.format(today)+'.dat'
        print 'outputFileName = ',outputFileName
        ...
        outputFile.write ('%s' % totalTitle)
        outputFile.write ('\n#  Total Particles:  %d' % totalPrtcl)
        outputFile.write ('\n#  Losted Particles: %d' % lostedTotal)
        outputFile.write ('\n#  R_init:  {:.3f} mm'.format(initialRadius))
        if totalTurns != dataPointsCounter:
            outputFile.write ('\n#  Each point correponds to {} turns'. \
                              format(int(totalTurns/dataPointsCounter)))

        strTitle = "\n#\n#  Turn   Losted Particles\n#\n"
        outputFile.write ('%s' % strTitle)
        for turn in range(totalTurns):
            strLine = '  {: 5d}'.format(turn)+'  '+'{: 12d}'.format(int(lostedParticles[turn]))
            outputFile.write ('%s\n' % strLine)
        ...
        if totalTurns == dataPointsCounter:
            ...
            strTitle = "#\n# Point   Turn   "
        strTitle += "R > R_init    R > 2 R_init   R > 3 R_init   R > 4 R_init   R > 5 R_init\n#\n"
        outputFile.write ('%s' % strTitle)
        for point in range(dataPointsCounter):
            strLine = '  {: 5d}'.format(point)
            if totalTurns != dataPointsCounter:
                strLine += ' {:5d}'.format(int(point*turnsPerPoint))
            strLine +='  '+ '{: 12d}'.format(int(prtclsOutOfArea[point,0]))+'  '+ \
                      '{: 12d}'.format(int(prtclsOutOfArea[point,1]))+'  '+ \
                      '{: 12d}'.format(int(prtclsOutOfArea[point,2]))+'  '+ \
                      '{: 12d}'.format(int(prtclsOutOfArea[point,3]))+'  '+ \
                      '{: 12d}'.format(int(prtclsOutOfArea[point,4]))
            outputFile.write ('%s\n' % strLine)
...
#
    ...
    saveDataAfterTurns = int(raw_input( \
    '\nPeriodicity (in turns) to save multiturn data \n (= -1 to interrupt simulation):'))
    if saveDataAfterTurns == -1:
        return
    ...
    eachTurnOutputFlag = int(raw_input('\neachTurnOutputFlag (0 - no, 1 - yes, -1 - to interrupt simulation):'))
    if eachTurnOutputFlag == -1:
        return

    outputFlag = int(raw_input('\noutputFlag to check update the stengths of the lenses \n(0 - no, 1 - yes, -1 - to interrupt simulation):'))
    if outputFlag == -1:
        return

    saveAllResultsFlag = int(raw_input( \
    '\n \nSave all results in the files (0 - no, 1 - yes, = -1 to interrupt simulation):'))
    if saveAllResultsFlag == -1:
        return
...
# Input data for simulation:
    ...
    totalTitle += "\n###"
    totalTitle += "\n###     Structure: {}".format(fileIOTA)
    totalTitle += "\n###"
    totalTitle += "\n###     Particles in the bunch = {}".format(particlesInBunch)
    totalTitle += "\n###     Total number of turns = {}".format(totalTurns)
    totalTitle += "\n###     Number of turns for ramp = {}".format(rampTurns)
    totalTitle += "\n###     Periodicity (in turns) to update the parameters = {}".format(updateAfterTurns)
    totalTitle += "\n###     magnificationType = {}".format(magnificationType)
    if magnificationType == 1:
        totalTitle +="\n###     Factor of relative magnification (RM) = {}".format(mgnfctnFctr)
    if magnificationType == 2:
        totalTitle +="\n###     For absolute magnification (AM) initial value t_i = {}".format(t_i)
        totalTitle +="\n###     For absolute magnification (AM) final value t_f = {}".format(t_f)
    laws = ['linear', 'parabolic', 'smooth sign-function']
    totalTitle += "\n###     Law of magnification: {}".format(laws[law-1])
    totalTitle += "\n###     Steps of ramps: {}".format(stepsInMgnfctn)
    totalTitle += "\n###     Round aperture: {}".format(roundApertureLost)+" mm"
    totalTitle += "\n###"
    totalTitle += "\n###        Predefined parameters for nonlinear insertion:"
    totalTitle += "\n###"
    totalTitle += "\n###     Length, l0 = {}".format(insrtn_l0)+" m"
    totalTitle += "\n###     Phase, mu0 = {}".format(insrtn_mu0)+" rad (/2pi)"
    totalTitle += "\n###     Aperture factor, cval = {}".format(insrtn_c)+" m^(1/2)"
    totalTitle += "\n###     Number of lens inside insertion = {}".format(num_lens)
    totalTitle += "\n###"
    totalTitle += "\n################################################################"
...
#        print "First lens has maximal knll"  
    else:
        startSequenceLenses = 1              # First lens has minimal knll
#        print "First lens has minimal knll"  
...
#     printAttributes(bunchParticles,'bunchParticles', 'bunch.get_local_particles()')
    plotcoordDistr(bunchParticles,0,0,0.,0,"")
#--------------------------------------------------
...
#                  this field can be used as "loss flag": 
#                  particle is lost (if value > 0) or not (if value = 0), 
#              1 - maximum of crrntR,
#              2 - turn, when crrntR has maximum
#
    lostedParticles = np.zeros((particlesInBunch,3))
# Number of losted particle for each turn. 
# Particle is lost if its radius > roundAperure:
    numbLostedParticles = np.zeros(totalTurns)
# 2D-Array 'prtclsOutOfArea' describes for each turn the number
...
#
    ...
    cnll_lenses = np.zeros(stepsInMgnfctn)
    t_lenses = np.zeros(stepsInMgnfctn)
    totalLostParticles = 0
    nUpdate = 1
    stepOfMgnfcn = 1
    tracksTurnsCounter = saveDataAfterTurns
    dataPointsCounter = 0
    totalTimeCPU = 0.
...
#
            if magnificationType == 1:
                knll_lenses[0] = from_t_to_knll(insrtn_l0, insrtn_mu0, strengthLens[0], \
                                   insrtn_c, num_lens)
                t_crrnt = from_knll_to_t(insrtn_l0, insrtn_mu0, knll_lenses[0], \
                                   insrtn_c, num_lens)
                ramp_actions = Ramp_actions(magnificationType,0,strengthLens, \
                                            outputFlag,controlName)   
            if magnificationType == 2:
                dataInsertion = \
                NonlinearInsertion(insrtn_l0, insrtn_mu0, strengthLens[0], \
                                   insrtn_c, num_lens).generate_lens(startSequenceLenses)
                knll_lens = dataInsertion.knll
                cnll_lens = dataInsertion.cnll
#                print "For turnCrrnt = 0: cnll = ",cnll_lens
                knll_lenses[0] = knll_lens[positionControlLens]
                t_crrnt = from_knll_to_t(insrtn_l0, insrtn_mu0, knll_lenses[0], \
                                   insrtn_c, num_lens)
#                print "For turnCrrnt = 0: t_crrnt = ",t_crrnt
                ramp_actions = Ramp_actions(magnificationType,0,knll_lens,outputFlag,controlName)   
...
#        print "Start of turn = ",turnCrrnt+1
        ...
        if turnCrrnt == 0:
            plotcoordDistr(bunchParticles,1,0,t_crrnt,saveAllResultsFlag,totalTitle)
...
# Calculation of the vertical quadrupole momentum of the bunch:
        ...
        if tracksTurnsCounter == saveDataAfterTurns:
            for prtcl in range(numbPartcls):
                for k in range(2):
                    tracksNonLinear[dataPointsCounter,prtcl,k] = coordsTracks[prtcl,k]
#                if prtcl < 3:
...
# Number of particles outside the areas with different sizes:
                for m in range(5):
                    if crrntRadius > (5-m)*initialRadius:
                        prtclsOutOfArea[int(dataPointsCounter),4-m] += 1
            dataPointsCounter += 1
        tracksTurnsCounter += 1
        for prtcl in range(numbPartcls):
# Checking of the lost particles (outside of round aperture for each turn!):
            crrntRadius = np.sqrt(coordsTracks[prtcl,0]**2 + coordsTracks[prtcl,1]**2)
            ...
            if crrntRadius > roundApertureLost:
                if lostedParticles[prtcl,0] == 0:
                    lostedParticles[prtcl,0] = int(turnCrrnt)+1   # counter of turns begins from 1!
...
#            t_crrnt = from_knll_to_t(insrtn_l0, insrtn_mu0, knll_lenses[0], \
#                                   insrtn_c, num_lens)
#            print "For turnCrrnt = {}: \nt_ccnt ={} \n".format(turnCrrnt,t_crrnt)
            plotcoordDistr(bunchParticles,0,turnNumber,t_crrnt,saveAllResultsFlag,totalTitle)
#== #
...
#   
                ...
                knll_lens = dataInsertion.knll
#                cnll_lens = dataInsertion.cnll
#                print "cnll = ",cnll_lens
                knll_lenses[stepOfMgnfcn] = knll_lens[positionControlLens]
                t_crrnt = from_knll_to_t(insrtn_l0, insrtn_mu0, \
                                         knll_lenses[stepOfMgnfcn],insrtn_c, num_lens)
# For checking:
                print "Step ",stepOfMgnfcn,"(abs): knll_lenses[",stepOfMgnfcn, \
                       "] = ",knll_lenses[stepOfMgnfcn]," --> t=",t_crrnt
# For checking:
#                if stepOfMgnfcn > 0:
...
#    print "\nloc_num = ",loc_num
#    t_crrnt = from_knll_to_t(insrtn_l0, insrtn_mu0, knll_lenses[0],insrtn_c, num_lens)
    plotcoordDistr(bunchParticles,1,totalTurns,t_crrnt,saveAllResultsFlag,totalTitle)
    print ""
    plotTracks(tracksNonLinear,5,dataPointsCounter,saveDataAfterTurns)
    print ""
    plotResult_1(knll_lenses,t_lenses,secondVertBeamMomentum,stepsInMgnfctn,totalTurns, \
                 saveAllResultsFlag,totalTitle)
...
#            print turn,": ",numbLostedParticles[turn]
    ...
    plotResult_2(numbLostedParticles,prtclsOutOfArea,totalTurns,dataPointsCounter,particlesInBunch, \
                 roundApertureLost,initialRadius,saveAllResultsFlag,totalTitle)
...
# To recognize attributes of 'bunchParticles':
#     printAttributes(bunchParticles,'bunchParticles', 'bunch.get_local_particles()')
plotcoordDistr(bunchParticles,1,0,1.5e-6,0,"")
