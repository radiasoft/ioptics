...
# Putput: <synergia.utils.parallel_utils.Commxx at 0x7f228788edb8>
...
# 
# Note, for the kV-like distribution in coordinate space, the RMS value in x/y is 1/2 the maximum value. 
# That is, in coordinate space, $\sigma_w = 0.5 w_{max}$. For the Gaussian distribution in momentum space, 
# the RMS value is simply the standard deviation ($\sigma_w = \sigma_{p_w}$). The RMS emittance of the 
# bunch (geometric) is then the product of these two values $\epsilon_w = \sigma_w \times \sigma_{p_w}$.
# 
# Thus, we have $\epsilon_w = 0.5 w_{max} \sigma_{p_w}$. So if we specify a maximum coordinate (e.g. $x_{max}$), 
# then we can define $\sigma_{p_w} = 2 \epsilon_w / w_{max}$
# 
# For a beam not at a waist, the beam should have a coordinate rotation applied by the twiss parameter matrix 
# in each plane, but since we are injecting at beam waists we don't have to worry about this.
# 
# Defining $x_{max}$. This follows from the convention of defining $\epsilon_{x} = \sigma_x^2/\beta_x$. For a 
# KV-like distribution in coordinate space, we can relate the maximum x value to the RMS via 
# $\sigma_x = \sqrt{2}/2 x_{max}$. Thus, we have $x_{max} = \sqrt{2 \beta_x \epsilon_x}$.
#
#  End of comments
#------------------------------ 
